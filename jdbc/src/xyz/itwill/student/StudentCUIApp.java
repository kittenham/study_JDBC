...
public class StudentCUIApp {
	...
	public StudentCUIApp() {
		...
		while(true) {
			...
			int choice;
			try {
				System.out.println("메뉴선택[1~6] >>");
				choice=Integer.parseInt(in.readLine()); //키보드로 문자열을 입력받아 정수값으로 변환하여 변수에 저장
				if(choice<1||choice>6) throw new Exception(); //메뉴선택을 잘못한 경우를 인위적으로 예외를 발생시킨 것.
			} catch (Exception e) {
				System.out.println("[에러] 메뉴를 잘못 선택하셨습니다.");
				System.out.println();
				continue; //while 구문(반복문) 재실행
			}
			System.out.println();
			
			if(choice==6) break; //반복문(While) 종료
			
			//메뉴 선택에 따른 기능을 구현 - 메소드 호출
			switch(choice) {
	...
	//=> 키보드로 학번을 제외한 나머지값을 입력받아 STUDENT 테이블에 저장된 학생정보를 변경하고 처리결과를 출력하는 메소드
	...
	public void displayAllStudent() {
		...
		List<StudentDTO> studentList = StudentDAOImpl.getDAO().selectAllStudent();
			//studentList 객체 안에는 학생정보들이 저장되어 있을 것.
		...
		for(StudentDTO student : studentList) {
			System.out.println(student); //StudentDTO 클래스의 toString() 메소드 자동 호출 - StudentDTO 객체의 필드값을 반환받아 출력
		}
		System.out.println("==================================================================");
