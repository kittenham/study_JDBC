...
//ex. 웹사이트의 회원가입, 탈퇴 등을 할때 사용자 정보를 연동시키는 기능을 할 수 있음.

//java.sql : JDBC 기능의 프로그램을 작성하기 위한 클래스 및 인터페이스가 선언된 패키지
//=> java.sql 패키지에서는 JDBC 기능 구현을 위해 "인터페이스"를 제공한다. - DBMS 종류가 다양하므로 클래스로 제공이 불가능하기 때문.
...
//			(로컬컴퓨터의 자바폴더에 들어가서 lib 폴더에 직접 파일(jar)을 넣는 방법도 있음(권장은X))
//**이 작업을 프로젝트를 할때마다 해줘야함(프로젝트에 필요한 라이브러리가 다 다르기 때문)
...
public class InsertStudentApp {
	public static void main(String[] args) {
		...
		Statement stmt = null;
		try {
			//[순서1]. Oracle Driver 클래스로 객체를 생성하여 DriverManager 클래스의 JDBC Driver 객체로 등록 
			//(사용자가 직접하는 것이 아니라 클래스만 읽어들이면 알아서 해줌)
			// => 따라서, OracleDriver 클래스를 읽어 메모리에 저장하여 자동으로 Oracle 객체를 생성하여 
			//DriverManager 클래스의 JDBC Driver 객체로 등록 하는 것이 더 효율적이다.
...
//			DriverManager.registerDriver(new OracleDriver());	==> 사용자가 직접작성한것(수동)
			...
			Class.forName("oracle.jdbc.driver.OracleDriver");
				...
				//	 DriverManager 클래스의 JDBC Driver로 등록하는 메소드를 호출한다. (효율적인 방법(자동이기때문)) : Oracle > JDBC
				//수동으로 여러번쓰면 쓴만큼 실행이 되어서 메모리 낭비가 되는데, 자동으로 쓰면 여러번 써도 한번만 실행됨.(훨씬 효율적)
			
			//[순서2]. DriverManager 클래스에 등록된 JDBC Driver 객체를 이용하여 DBMS 서버에 접속해 Connection 객체를 반환받아 저장.
			//DriverManager.gtConnection(String url, String user, String password)
			// => DriverManager 클래스에 등록된 JDBC Driver 객체를 이용하여 DBMS 서버에 접속
			// => DBMS 서버에 정상적으로 접속이 되면, 접속된 정보가 저장된 Connection 객체를 반환
